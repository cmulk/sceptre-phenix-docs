...
that reaches the server over the Unix socket normally keeps its **own** bridge
mode; the server's value does not replace it. The single exception is `auto`:
if the server is running in `auto` mode, the client is forced into `auto` too,
even if `manual` was set locally. This makes the server's isolation policy a
floor that CLI users cannot opt out of.

| Server mode | Client mode | Effective mode |
| :--- | :--- | :--- |
| `auto` | `manual` | `auto` (server wins) |
| `auto` | `auto` | `auto` |
| `manual` | `auto` | `auto` (client wins) |
